...
class CoreEditingEngine:
    ...
    def generate_video(self, schema: Dict[str, Any], output_file, logger=None) -> None:
        ...
        if logger:
            ...
            video.write_videofile(
                output_file, codec="libx264", audio_codec="aac", logger=my_logger
            )
        else:
    ...
    def process_audio_actions(
        self, clip: AudioFileClip, actions: List[Dict[str, Any]]
    ) -> AudioFileClip:
        clip = self.process_common_actions(clip, actions)
        for action in actions:
